...
class ConfigModel:
    ...
    def get_max_dimensions(self) -> Tuple[int, int]:
        return self.config_data.max_width, self.config_data.max_height

    def get_dpi(self) -> int:
        return self.config_data.dpi
...
class ImageModel:
    ...
    def update_data(self, images: List[np.ndarray]) -> None:
        max_width, max_height = self.config_model.get_max_dimensions()
        self.image_data.original_sized_images = images
        self.image_data.images = resize_images(images, max_width, max_height)
    ...
    def load_images(self, path: str) -> None:
        try:
            images = load_pdf(path, self.config_model.get_dpi())
...
class MaskModel(RedoUndoInterface):
    ...
    def get_bgr_mask(self) -> np.ndarray:
        if self.config_model.get_mode() == MaskMode.THRESHOLD:
    ...
    def get_weighted_image(self) -> np.ndarray:
        ...
        if current_image is None or self.mask_data.final_mask is None:
        ...
        blended_image = (1 - self.config_model.get_weight()) * current_image.astype(np.float32) + \
                        self.config_model.get_weight() * self.get_bgr_mask().astype(np.float32)
...
class ParameterModel:
    ...
    def get_processed_current_image(self, mask_model: MaskModel) -> np.ndarray:
        current_image = self.image_model.get_current_image()
        lower = np.array([self.current_parameters.b_min,
                          self.current_parameters.g_min,
                          self.current_parameters.r_min], dtype=np.uint8)
        upper = np.array([self.current_parameters.b_max,
                          self.current_parameters.g_max,
                          self.current_parameters.r_max], dtype=np.uint8)

        bgr_mask = mask_model.get_bgr_mask()
        ...
        processed_image = sharpen_image(processed_image, self.current_parameters.w / 10)
...
class BaseModel(RedoUndoInterface):
    def __init__(self, images=None, dpi=200, max_width=1920, max_height=1080):
        self.config_model = ConfigModel(max_width, max_height, dpi)
        self.image_model = ImageModel(self.config_model)
        self.mask_model = MaskModel(self.image_model, self.config_model)
        self.parameter_model = ParameterModel(self.image_model, self.config_model)
    ...
    def get_image_to_show(self):
        if self.config_model.get_mode() == MaskMode.ADJUST:
            ...
            return self.mask_model.get_weighted_image()
    ...
    def get_processed_current_image(self) -> np.ndarray:
        ...
        bgr_mask = self.mask_model.get_bgr_mask()
        mask = cv2.bitwise_and(current_image, bgr_mask)
        gray_mask = cv2.inRange(mask, lower, upper)
        gray_mask = cv2.bitwise_and(gray_mask, cv2.cvtColor(bgr_mask, cv2.COLOR_BGR2GRAY))
